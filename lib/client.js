...
// v0.1.2：右键设置页分区导航（settings.section 的投影）→ 弹出勾选菜单 →
	...
	window.__ModuleLoader__.load({
		...
		factory: (require) => {
			...
			function apply(ctx) {
				...
				// 菜单自有文案多语言化：经 locale 服务注册 namespace 字典（zh/en 键集一致），
				// bind 稳定 translate fn（按调用时 active locale 求值）。分区 label 已由
				// resolveSlotLabel 本地化，这里只覆盖菜单自身的 4 串（title/warn/showAll/hideAll）。
				// locale 缺席 / 面异常 → 静默降级到内置英文，菜单可读不崩。
				var NAMESPACE = 'dsh-setting-manager';
				var FALLBACK_STRINGS = {
					title: 'Settings Menu',
					warn: 'All sections are hidden',
					showAll: 'Show All',
					hideAll: 'Hide All'
				};
				if (locale && typeof locale.register === 'function') {
					try {
						...
						var offZh = locale.register(NAMESPACE, 'zh', zhDict);
						var offEn = locale.register(NAMESPACE, 'en', enDict);
						if (offZh) cleanupFns.push(function () { offZh(); });
						if (offEn) cleanupFns.push(function () { offEn(); });
					} catch (err) {
						void err;
					}
				}
				...
				if (locale && typeof locale.bind === 'function') {
					try {
						strT = locale.bind(NAMESPACE);
					} catch (err) {
						void err;
						strT = null;
					}
				}
				/** 菜单文案取值：locale 命中用 t(key)（回译出原 key 视为未注册）；否则回退内置英文。 */
				function str(key) {
					if (strT) {
						try {
							var v = strT(key);
				...
				function renderMenu(allHidden) {
					...
					showAll.textContent = str('showAll');
					...
					hideAll.textContent = str('hideAll');
