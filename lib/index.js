...
// v0.1.2：右键设置页「设置菜单」（settings.section 分区导航）→ 勾选菜单 →
...
/** 原子写持久化状态：每次调用唯一 tmp 名（FILE + '.tmp-' + 进程 pid + 单调递增序号，
 * 并发写各占独立 tmp，互不覆盖），先写 tmp 再 rename 覆盖 FILE
 * （同文件系统 rename 原子，避免写一半损坏，后到 rename 胜）。
 * 写 / rename 失败时 best-effort 清理自己的 tmp 后 rethrow。
 * 入参形状恒为 { version: 1, hidden: [...] }。 */
